...
class DDPGAgent():
    def __init__(self, index, config, filenames=None):
        ...
        self.actor_local = Network(config.actor, config.general.seed, device)
        self.actor_target = Network(config.actor, config.general.seed, device)
        self.actor_optimizer = Adam(self.actor_local.parameters(), lr=config.actor.lr)
        self.critic_local = Network(config.critic, config.general.seed, device)
        self.critic_target = Network(config.critic, config.general.seed, device)
