    ...
    def __init__(self, config, file_prefix=None):
        ...
        if file_prefix:
            for i, to_load in enumerate(self.agents):
                ...
                actor_file = torch.load(f"{os.getcwd()}/models/{file_prefix}_actor_{i}.weights", map_location='cpu')
                critic_file = torch.load(f"{os.getcwd()}/models/{file_prefix}_critic_{i}.weights", map_location='cpu')
